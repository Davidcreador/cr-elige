  ...
  const getCandidateContent = (candidate: Candidate | undefined, section: 'priorities' | 'economicFiscal' | 'socialPrograms' | 'infrastructure') => {
    ...
    return (
      <ReactMarkdown
        ...
        components={{
          p: ({ children }) => <p className="mb-3 prose-sm">{children}</p>,
          ul: ({ children }) => <ul className="space-y-1 prose-sm">{children}</ul>,
          ol: ({ children }) => <ol className="space-y-1 prose-sm">{children}</ol>,
          li: ({ children }) => <li className="prose-sm">{children}</li>,
          strong: ({ children }) => <strong className="font-semibold text-gray-900 prose-sm">{children}</strong>,
          em: ({ children }) => <em className="italic text-gray-600 prose-sm">{children}</em>,
        }}
      >
        {content}
      </ReactMarkdown>
